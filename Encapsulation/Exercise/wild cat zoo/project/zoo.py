...
class Zoo:
    ...
    def tend_animals(self):
        cost_for_animals = sum(a.money_for_care for a in self.animals)
        if self.__budget < cost_for_animals:
            return "You have no budget to tend the animals. They are unhappy."
        self.__budget -= cost_for_animals
    ...
    @staticmethod
    def __print_status(category_list: list[Union[Animal, Worker]], *args: str) -> str:
        elements = {arg: [] for arg in args} # {"Lion": [], "Tiger": []}
        for item in category_list:
            elements[item.__class__.__name__].append(item.__repr__())
